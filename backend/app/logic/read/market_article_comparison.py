...
def market_article_comparison(
    ...
    # Indications :
    # - Vérifier la présence du lien market_master_article_id (public.master_articles.market_master_article_id) et tracer les erreurs Supabase (réseau/colonnes manquantes).
    # - Mentionner la modification à prévoir : inclure les prix/quantités à 0 dans les moyennes (actuellement filtrés par la condition truthy) pour rester cohérent avec la DB.
    # - Remonter en option prix médian et nombre d'observations utilisateur/marché, et signaler si supplier_id n'est pas rattaché à l'établissement.
    # Tests robustes :
    # - Injecter des jeux de données avec prix manquants ou à 0, marché sans observations et articles utilisateur hors période.
    # - Couvrir un master_article introuvable et un market_master_article_id absent pour valider les erreurs retournées et la stabilité des diff_avg_price/potential_savings.
