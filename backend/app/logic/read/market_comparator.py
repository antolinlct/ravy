...
def get_month_bounds(target_date: Optional[date] = None):
    ...
    # Indications :
    # - Ajouter l'import from datetime import date manquant pour éviter un NameError et synchroniser la timezone avec les services consommateurs.
    # - Tester les bornes inclusives/exclusives et tracer la période effective pour garantir le même périmètre que les rapports marché.
    # Tests robustes :
    # - Vérifier le calcul des bornes sur changement de mois/année, y compris dates explicites du front et cas start_date > end_date bloqué en amont.
...
def _fetch_product_data(
    ...
    # Indications :
    # - Valider la disponibilité des colonnes Supabase (unit_price, date) et leur typage NUMERIC/DATE ; tracer les erreurs réseau.
    # - Mentionner la modification à prévoir : lorsque only_my_invoices=True, vérifier que master_articles renvoie aussi l'establishment_id pour éviter des fuites inter-établissement.
    # - Prévoir un cache court et, si besoin, renvoyer médiane/écart-type pour fiabiliser la comparaison.
    # Tests robustes :
    # - Mocker Supabase pour renvoyer des dates non triées, des prix None et des jeux volumineux afin de valider l'agrégation journalière triée et la performance.
    # - Tester only_my_invoices avec master_articles vides ou mal mappés pour s'assurer que count_purchases et stats restent cohérents.
...
def market_comparator(
    ...
    # Indications :
    # - Contrôler que les IDs marché proviennent du même scope établissement (public.market_master_articles) et journaliser les erreurs Supabase.
    # - Ajouter un garde-fou pour avg1=0 afin d'éviter les divisions par zéro et rappeler la source (marché vs mes factures) par produit.
    # - Vérifier que les dates utilisées correspondent à la timezone des services consommateurs.
    # Tests robustes :
    # - Simuler avg1=0, séries vides et combinaisons only_my_invoices True/False pour les deux produits afin de valider diff_avg_pct et la propagation des filtres.
    # - Injecter des meta manquantes pour l'un des produits pour s'assurer que le retour reste structuré malgré un scope incomplet.
