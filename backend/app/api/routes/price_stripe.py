...
def create_price_stripe(data: PriceStripe):
    created = price_stripe_service.create_price_stripe(data.dict())
    return PriceStripe(**created)

@router.patch("/{id}", response_model=PriceStripe)
def update_price_stripe(id: int, data: PriceStripe):
    updated = price_stripe_service.update_price_stripe(id, data.dict(exclude_unset=True))
    if not updated:
        raise HTTPException(status_code=404, detail="PriceStripe not found")
